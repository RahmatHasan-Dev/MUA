...
<head>
    ...
    <title>Kelola Fun Fact | MUA Admin</title>
        ...
        <div class="container pt-4">
            <div class="card shadow-0 border">
                <div class="card-header border-bottom py-3">
                    <h5 class="mb-0"><strong><i class="fas fa-newspaper me-2"></i>Daftar Fun Fact</strong></h5>
                </div>
                <div class="card-body">
                    <!-- Search & Export -->
                    <div class="row mb-4">
                        <div class="col-md-8">
                            <form action="{{ route('admin.funfact') }}" method="GET" class="d-flex gap-2">
                                <!-- Input Pencarian -->
                                <div class="input-group">
                                    <input type="text" name="search" class="form-control"
                                        placeholder="Cari Deskripsi, Angka, ..." value="{{ request('search') }}">
                                    <button class="btn btn-success" type="submit"><i
                                            class="fas fa-search"></i></button>
                                </div>

                                @if (request('search') || request('role'))
                                    <a href="{{ route('admin.funfact') }}" class="btn btn-danger px-3"
                                        title="Reset Filter"><i class="fas fa-times"></i></a>
                                @endif
                            </form>
                        </div>
                        <div class="col-md-4 text-end">
                            <a href="{{ route('admin.funfact.add') }}" class="btn btn-success"><i
                                    class="fas fa-plus me-2"></i>Tambah Fun Fact</a>
                            <a href="{{ route('admin.funfact.export') }}" class="btn btn-success"><i
                        ...
                        <table class="table table-hover align-middle mb-0 bg-white">
                            ...
                            <tbody>
                                @foreach ($funfact as $funfactx)
                                    <tr>
                                        <td>{{ $funfactx->emoji }}</td>
                                        <td>{{ $funfactx->headline }}</td>
                                        <td>{{ $funfactx->angka }}</td>
                                        <td>{{ $funfactx->deskripsi ?? '-' }}</td>
                                        <td>

                                            <a href="{{ route('admin.funfact.edit', $funfactx->id_funfact) }}"
                                                class="btn btn-warning btn-sm mb-1"><i class="fas fa-edit"></i>

                                            </a>

                                            <form action="{{ route('admin.funfact.delete', $funfactx->id_funfact) }}"
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-4">
                        <div>
                            <p class="mb-0 text-muted">
                                Showing {{ $funfact->firstItem() }} to {{ $funfact->lastItem() }} of
                                {{ $funfact->total() }} results
                            </p>
                        </div>
                        <div>
                            {{ $funfact->links('pagination::bootstrap-5') }}
                        </div>
                    </div>
                </div>
            </div>
