...
<head>
    ...
    <title>Edit Visi Misi | MUA Admin</title>
        ...
        <div class="container pt-4">
            <div class="card shadow-0 border">
                <div class="card-header border-bottom py-3">
                    <h5 class="mb-0"><strong><i class="fas fa-edit me-2"></i>Edit
                            {{ ucfirst($visimisi->kategori) }}</strong></h5>
                </div>
                <div class="card-body">
                    @if ($errors->any())
                    ...
                    <form action="{{ route('admin.visimisi.update', $visimisi->id) }}" method="POST">
                        ...
                        <!-- Kategori Readonly agar tidak berubah -->
                        <div class="mb-4">
                            <label class="form-label">Kategori</label>
                            <input type="text" class="form-control" value="{{ ucfirst($visimisi->kategori) }}"
                                readonly disabled style="background-color: #e9ecef;">
                            <input type="hidden" name="kategori" value="{{ $visimisi->kategori }}">
                        </div>

                        <div class="mb-4">
                            <label class="form-label">Judul</label>
                            <input type="text" name="judul" class="form-control"
                                value="{{ old('judul', $visimisi->judul) }}" required />
                        </div>

                        <div class="mb-4">
                            <label class="form-label">Deskripsi</label>
                            <textarea name="deskripsi" class="form-control" rows="4" required>{{ old('deskripsi', $visimisi->deskripsi) }}</textarea>
                        </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    <span class="input-group-text"><i id="iconPreview"
                                            class="{{ old('icon', $visimisi->icon ?? 'bi bi-question-circle') }}"></i></span>
                                    <input type="text" name="icon" id="iconInput" class="form-control"
                                        value="{{ old('icon', $visimisi->icon) }}" placeholder="bi bi-tree" />
                                </div>
                                <small class="text-muted">Gunakan class dari <a href="https://icons.getbootstrap.com/"
                                        target="_blank">Bootstrap Icons</a></small>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Urutan</label>
                                <input type="number" name="urutan" class="form-control"
                                    value="{{ old('urutan', $visimisi->urutan) }}" />
                            </div>
                        </div>
